...
-(void)addDetailProductView{
    ...
    self.priceLabel = [[UILabel alloc]initWithFrame:CGRectMake(20, 64+330+50, 350, 30)];
    NSMutableString *priceStr = [NSMutableString string];
    [priceStr appendString:@"€"];
    [priceStr appendFormat:@"%@",self.product.price];
    ...
    NSString *str = [NSString stringWithFormat:@"€%.2f",[self.product.price floatValue]*[self.product.clickNumber integerValue]];
    self.bottomPrice.text = str;    [bottomView addSubview:self.bottomPrice];
    
    UIButton *increase = [[UIButton alloc]initWithFrame:CGRectMake(190, 13, 70, 35)];
    [increase setBackgroundImage:[UIImage imageNamed:@"详情_减@2x.png"] forState:UIControlStateNormal];
    [increase addTarget:self action:@selector(subPackage) forControlEvents:UIControlEventTouchUpInside];
    [bottomView addSubview:increase];
    
    UIButton *decrease = [[UIButton alloc]initWithFrame:CGRectMake(280, 13, 70, 35)];
    [decrease setBackgroundImage:[UIImage imageNamed:@"详情_加@2x.png"] forState:UIControlStateNormal];
    [decrease addTarget:self action:@selector(addPackage) forControlEvents:UIControlEventTouchUpInside];
