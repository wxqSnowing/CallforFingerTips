...
-(void)setProduct:(Product *)product{
    ...
    if (_product) {
        ...
        _discountLabel.text = product.discountPrice;
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        self.moneyLabel = [[UILabel alloc] initWithFrame:CGRectMake(moneyX, moneyY, (UISCREE_WIDTH - 80) / 4-20, 20)];

        self.moneyLabel.textAlignment = NSTextAlignmentLeft;
        [self addSubview:self.moneyLabel];
        UIView *view = [[UIView alloc] initWithFrame:CGRectMake(moneyX, moneyY+10, (UISCREE_WIDTH - 80) / 4-30, 2)];
        view.backgroundColor = [UIColor blackColor];
        [self addSubview:view];
        
        
        
        CGFloat discoundX = (UISCREE_WIDTH - 80) / 4;
        CGFloat discoundY = self.descLabel.frame.origin.y+self.descLabel.frame.size.height+15;
        self.discountLabel = [[UILabel alloc] initWithFrame:CGRectMake(discoundX, discoundY, (UISCREE_WIDTH - 80) / 4-20, 20)];
